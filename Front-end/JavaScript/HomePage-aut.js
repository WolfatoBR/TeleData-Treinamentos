...
function redirectToCategory(categorySlug) {
    // Redireciona para página de catálogo com parâmetro de categoria
    window.location.href = `/Front-end/Pages/Catalogo.html?categoria=${categorySlug}`;
}

// Alternativa: Adicionar evento de clique via JavaScript (mais limpo)
document.addEventListener("DOMContentLoaded", function () {
    ...
    categoryCards.forEach((card) => {
        card.addEventListener("click", function () {
            ...
            setTimeout(() => {
                window.location.href = `/Front-end/Pages/Catalogo.html?categoria=${category}`;
            }, 150);
        });
    });
});
